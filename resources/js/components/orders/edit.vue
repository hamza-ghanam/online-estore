<template>
    <div id="order-edit">
        <h2 class="mb-3">Modify an order</h2>
...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            try {
                let res = await fetch(`/api/order/${this.order_id}`);
                res = await res.json();

                if (res.code === 200) {
                    this.orderInputDto.customer_id = res.results.customer_id;
                    this.orderInputDto.orderDate = res.results.orderDate.substring(0, 10);
                    this.totalAmount = parseFloat(res.results.totalAmount).toFixed(2);

                    for (const item of res.results.products) {
                        this.addToOrderItems(item.id, item.pivot.quantity, item.pivot.unitPrice);
                    }
                }
            } catch (error) {
        ...
        methods: {
            ...
            addToOrderItems(productId, quantity = 0, unitPrice = 0) {
                const product = this.products.find((prod) => prod.id === productId);
                this.orderInputDto.orderItems.push({
                    prodName: product.productName,
                    product_id: product.id,
                    quantity,
                    unitPrice: unitPrice === 0 ? product.unitPrice : unitPrice,
                });
            ...
            deleteOrderItem(index) {
                ...
                this.totalAmount -= (deletedItem.quantity * deletedItem.unitPrice);
                this.totalAmount = parseFloat(this.totalAmount).toFixed(2);
