<template>
    <div id="prod-edit">
        <h2 class="mb-3">Modify a product</h2>
            ...
            <div class="col-4">
                <form method="post" @submit.prevent="updateProduct">
                    <fieldset>
                        ...
                        <div class="form-group">
                            <label for="supplier_id">Example select</label>
...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            try {
                let res = await fetch(`/api/product/${this.product_id}`);
                res = await res.json();

                if (res.code === 200) {
                    this.productInputDto = res.results;
                }
            } catch (error) {
